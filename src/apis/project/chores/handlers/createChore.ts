import { ObjectId } from 'mongodb';
import { toDocumentId, toQueryId } from '@/server/template/utils';
import { chores, smartLists } from '@/server/database';
import {
    API_CREATE_CHORE,
    type ApiHandlerContext,
    type CreateChoreRequest,
    type CreateChoreResponse,
} from '..';
import { clampLastCompleted, toChoreClient } from '../shared';

export const createChore = async (
    request: CreateChoreRequest,
    context: ApiHandlerContext
): Promise<CreateChoreResponse> => {
    try {
        if (!context.userId) return { error: 'Not authenticated' };
        if (!request.listId) return { error: 'listId is required' };
        const name = request.name?.trim();
        if (!name) return { error: 'Name is required' };
        const userId = toQueryId(context.userId) as ObjectId;

        const list = await smartLists.findListById(request.listId, userId);
        if (!list) return { error: 'List not found' };

        if (request._id) {
            const existing = await chores.findChoreById(request._id, userId);
            if (existing) return { chore: toChoreClient(existing) };
        }

        const now = new Date();
        const created = await chores.createChore({
            _id: request._id ? toDocumentId(request._id) : new ObjectId(),
            userId,
            listId: list._id,
            name,
            repeatIntervalDays: Math.max(1, request.repeat_interval_days),
            lastCompletedAt: clampLastCompleted(request.last_completed_at),
            createdAt: now,
            updatedAt: now,
        });
        return { chore: toChoreClient(created) };
    } catch (error) {
        console.error('createChore error', error);
        return { error: error instanceof Error ? error.message : 'Failed to create chore' };
    }
};

export { API_CREATE_CHORE };
